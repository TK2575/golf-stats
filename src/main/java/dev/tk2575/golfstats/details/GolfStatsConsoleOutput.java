...
public class GolfStatsConsoleOutput implements Runnable {
	...
	public void run() {
		try {
			//FIXME indicate no files in directory (log/exception message) but gracefully output empty list
			...
			PerformanceSummary performanceSummary = new PerformanceSummary(
					List.of(simpleRounds, holeByHoleRounds, shotByShotRounds)
							.stream()
							.flatMap(List::stream)
							.collect(toList())
			);
			log.info(performanceSummary);
		}
		catch (Exception e) {
			log.error(e);
		}
